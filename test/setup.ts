...
import { DssInstance, getDss } from "./dss/dss";
import {
...
export async function setup(load: boolean, setupConfig: SetupConfig) {
  ...
  let mockStarknetMessaging: Address;
  ...
  if (load) {
    ...
    } = await loadSetup(setupConfig, admin, snOwner, snCfg));
  } else {
    ({
      teleport,
      snTeleport,
